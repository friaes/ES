...
@DataJpaTest
class GetAssessmentsByInstitutionTest extends SpockTest {
    def assessment
    def assessmentDto
    def volunteer
    def institution
    ...
    def "get assessment by institution id, when no assessments exist"() {
        when:
    ...
    def "get assessment by institution id"() {
        when:
        ...
        then:"result"
        result.size() == 1
        result.get(0).getId() == assessment.getId()
    }

    def "create assessment with null id for institution"() {
        when:
        assessmentRepository.save(assessment)
        def result = assessmentService.getAssessmentsByInstitution(null)
    ...
    def "create assessment with invalid id for institution"() {
        when:
        assessmentRepository.save(assessment)
        def result = assessmentService.getAssessmentsByInstitution(100)
