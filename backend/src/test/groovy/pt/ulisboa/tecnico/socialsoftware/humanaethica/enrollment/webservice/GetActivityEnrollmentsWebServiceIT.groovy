...
class GetActivityEnrollmentsWebServiceIT extends SpockTest {
    ...
    def "get activity enrollments, no enrollments"() {
        given: "login as member"
        demoMemberLogin()

        when: "get activity enrollments"
    ...
    def "get activity enrollments, with enrollments"() {
        given: "login as member"
        demoMemberLogin()
        ...
        then: "check response"
        response.size() == 1
        response.get(0).motivation == "motivation to max"
